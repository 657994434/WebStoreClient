...
<body>
	...
	<div class="product">
		<div class="mylist">
			...
			<div class="right">
				<div class="myblance_top">
							<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您的余额</p>
					</div>
					<div class="myblance_info">
					<table>
					<form name='myblanceFrm' class='myblanceFrm' action='' method='post'>
							 <th>亲爱的<span>${user.userName}</span></th>
							 <tr><td>您当前余额为:</td><td><span>${user.userMoney}</span>(元)</td></tr>
							 <tr><td><span></span><a id="recharge_btn" href="#">充值</a></td><td><span></span><a id="transfer_btn" href="#">转账</a></td></tr>
						 </form>
					 </table>
			</div>
			</div>
	...
	<div class="recharge">
		<table>
			<th>充值</th>
			<tr><td width="40%" align="right">您要充值的金额</td><td aligh="left"><input id="money" type="text">(元)</td></tr>
			<tr><td align="right"><input id="money_btn" type="button" value="确  定"><span></span><input id="cancel_btn" type="submit" value="取  消"></td></tr>
		</table>
	</div>
	<script type="text/javascript">
		$(function(){$("#money_btn").click(function(){
				var money = $("#money").val();
				var param = "^[1-9]{1}[0-9]{2,8}";
				if($("#money").val()!=$("#money").val().match(param)){
					 alert("请输入正确的金额数量");
				}else{
					 $.post("<%= path%>/MoneyAction_recharge",{userId:"${user.userId}",userMoney:money},
		      	    	      function hasDataToDeal1(data){
		      	    	              if(data == "true"){     
		          	    	              alert("充值成功");  
		          	    	              window.location.href = "<%= path%>/jsp/myblance.jsp";	          	   
		         	    	                  }else{             
		      	    	                	alert("充值失败");
		      	    	                  }          
		      	    	    })
	...
	<div class="transfer">
		<table>
			<form id="transferFrm" action="">
			<th>转账</th>
			<tr><td width="40%" align="right">您要转账的用户</td><td aligh="left"><input id="account" type="text" onblur='if (this.value ==""){this.value="请输入转账用户"}' onfocus='if (this.value =="请输入转账用户"){this.value =""}' value='请输入转账用户'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
			<tr><td width="40%" align="right">您要转账的金额</td><td aligh="left"><input id="money2" type="text">(元)</td></tr>
			<tr><td align="center"><input type="submit" value="确  定"><span></span><input id="cancel_btn2" type="submit" value="取  消"></td></tr>
			</form>
		</table>
	</div>
	<!--footer-end-->
...
<script>
	 $(function(){
		 $(".recharge").hide();
		 $('#recharge_btn').click(function (event){
					 //取消事件冒泡
					event.stopPropagation();
					//设置弹出层的位置
					var offset = $(event.target).offset();
					 $(".recharge").css({ top: offset.top + $(event.target).height() + "px", left: offset.right });
					 //按钮的toggle,如果div是可见的,点击按钮切换为隐藏的;如果是隐藏的,切换为可见的。
							$('.recharge').toggle('slow');
...
<script>
	 $(function(){
		 ...
		 $('#transfer_btn').click(function (event){
					...
					//设置弹出层的位置
					var offset = $(event.target).offset();
					 $(".transfer").css({ top: offset.top + $(event.target).height() + "px", left: offset.right });
					 //按钮的toggle,如果div是可见的,点击按钮切换为隐藏的;如果是隐藏的,切换为可见的。
					 $('.transfer').show('slow');
